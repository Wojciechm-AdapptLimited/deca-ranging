...
void reportTOF(instance_data_t *inst)
{
        ...
        tof = convertDeviceTimeToSeconds64(tofi);
            ...
            case DWT_SIG_RX_NOERR :
				{
					...
					fprintf(instance_data[instance].dwlogdata.accumLogFile, "\n%02X %02X Rx time = %4.15e %02X%08X\n", fc /*FC*/, sn /*SN*/,
                            convertDeviceTimeToSecondsU64(dw_event->timeStamp), rxadj_hi16, rxadj_low32) ; // DEBUG
					fprintf(instance_data[instance].dwlogdata.accumLogFile, "\n%02X %02X Rx time(un) = %4.15e %02X%08X\n", fc /*FC*/, sn /*SN*/,
                            convertDeviceTimeToSecondsU64(rxUnTimeStamp), rxunadj_hi16, rxunadj_low32) ; // DEBUG
...
int _instanceaccumlogenable(int logEnable)
{
    ...
    {
        strncpy(s, DECA_ALLACCUM_LOG_FILE, sizeof(fnbuf)-(s-fnbuf)) ;
    }
	else if(logEnable == LOG_SINGLE_ACC)
	{
		 strncpy(s, DECA_ACCUM_LOG_FILE, sizeof(fnbuf)-(s-fnbuf)) ;
	}
    else
    {
        strncpy(s, DECA_ERRACCUM_LOG_FILE, sizeof(fnbuf)-(s-fnbuf)) ;
    }

    if ((instance_data[instance].dwlogdata.accumLogFile = fopen(fnbuf,"wt"))==NULL)  // open the file (allow reading by text editor)
    {
