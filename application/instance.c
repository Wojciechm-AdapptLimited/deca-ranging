...
int testapprun(instance_data_t *inst, int message, uint32 time_ms)
{
    ...
    switch (inst->testAppState)
    {
                        ...
                        switch(fcode)
                        {
							case RTLS_DEMO_MSG_RNG_INIT:
                            {
								if(inst->mode == TAG_TDOA) //only start ranging with someone if not ranging already
								{
                                    ...
                                    inst->finalReplyDelay = convertMicrosecondsToDeviceTime(final_reply_delay_us);
		...
		{
			double a1 = convertDeviceTimeToSecondsU64(Ra) * 1e9 - 150e6;
			double a2 = convertDeviceTimeToSecondsU64(Db) * 1e9 - 150e6;
			double b1 = convertDeviceTimeToSecondsU64(Rb) * 1e9 - 150e6;
			double b2 = convertDeviceTimeToSecondsU64(Da) * 1e9 - 150e6;
...
void instance_init_timings(void)
{
    ...
    inst->rnginitReplyDelay = convertMicrosecondsToDeviceTime(RNG_INIT_REPLY_DLY_MS * 1000);
    // Delay between poll reception and response transmission.
    inst->responseReplyDelay = convertMicrosecondsToDeviceTime(inst->anc_resp_dly_ms * 1000);
