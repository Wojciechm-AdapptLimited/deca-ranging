...
int spilogopen(void) {
    ...
    strcpy(s, DECA_SPI_DEBUG_LOG_FILE);               // then tag on rest of file name

    spiLog = fopen(fnbuf, "wt");    // open the file
...
// These functions are called from the application when run-time debug log enable is needed
...
int openSPI_VCP(void) {
    ...
    opened = findOpenEVB1000Port(); //find a COM port connected to EVB1000
...
int setSPIBitrate_VCP(int desiredRatekHz) {
    int x = vcSPISetSPIBitrate(desiredRatekHz);
...
int closeSPI_VCP(void) {
    ...
    return closePort((vcpHandle & 0xFF)); //find a COM port connected to EVB1000
} // end closeSPI_VCP()
...
int writetospi_vcp
    ...
    uint8* txBuffer;
    uint8* rxBuffer;
    uint32 totallength = 7 + bodylength + headerLength;
    ...
    error = writeData((char*) txBuffer, totallength, &written);
    ...
    error = readData((char*) rxBuffer, 3, &read);
    ...
    if (read != 3) {
        ...
        uint16 index = (headerLength == 1) ? 0 : ((txBuffer[6 + 1] & 0x80) ? ((txBuffer[6 + 2] << 7) |
                                                                              (txBuffer[6 + 1] & 0x7F)) : (
                                                          txBuffer[6 + 1] & 0x7F));
...
int writetospi_ch
    ...
    if (ch_spi_queue_ss(handle, 7) != CH_OK)
        return DWT_ERROR;      // set select line, bit 0 is select line 0, value 1 sets it active (low)

    // Write message header

    if (ch_spi_queue_array(handle, headerLength, headerBuffer) != headerLength)
        return DWT_ERROR; // Queue the array to be sent
    ...
    if (ch_spi_queue_array(handle, bodylength, bodyBuffer) != bodylength)
        return DWT_ERROR; // Queue the array to be sent
    ...
    if (ch_spi_batch_shift(handle, 0,
                           (void*) 0)                    // shift queued bytes/line changes out SPI, capture no bytes in return
        != headerLength + bodylength)
        return DWT_ERROR;          // and check it went okay
...
int readfromspi_vcp
    ...
    error = readData((char*) rxBuffer, readlength + 3, &read);
    ...
    if (read != (readlength + 3)) {
        ...
        error = writeData((char*) "deca?", 5, &written);

        Sleep(2);

        while (left > 0) {
            error = readData((char*) rxBuffer + readPtr, left, &read);
            readPtr += read;
            left -= read;
        }
    ...
    if ((rxBuffer[0] == 0x2) && (rxBuffer[1] == 0x0) && (rxBuffer[readlength + 2] == 0x3)) {
        ...
        printf("rxBuffer[last3] = %02X %02X %02X\n", rxBuffer[readlength], rxBuffer[readlength + 1],
               rxBuffer[readlength + 2]);
        //error = readData((char*) rxBuffer, readlength+3, &read);
...
int readfromspi_ch
    ...
    if (CH_OK != (res = ch_spi_queue_ss(handle, 7)))
        return DWT_ERROR;      // set select line, bit 0 is select line 0, value 1 sets it active (low)
    ...
    if (ch_spi_queue_byte(handle, readlength, 0x00) != readlength)
        return DWT_ERROR;   // send dummy bytes (zeros) for length we want to read back
    ...
    if (ch_spi_batch_shift(handle, readlength + headerLength,
                           rxBuffer) // shift queued bytes/line changes out SPI, capture RX butes into buffer
