...
void* hInst;                                // current instance
void* hFont;                                   // font handle
void* hWindow = nullptr;                                      // Main window handle
void* hMainPanel;                                // handle to main grey panel at bottom
void* hDebugPanel;                               // handle to debug panel
void* hVer = nullptr;
void* hTip = nullptr;
void* hPauseButton;
void* hStopLoggingButton = nullptr;
void* hAccLogHistButton = nullptr;
void* hAccLogHistButton2 = nullptr;
void* hAccLogSaveButton = nullptr;
void* hRoleComboBox;
void* hBlinkAdvise;                             // handle to blink advise text control
void* hDelayTextBox;                             // handle to antenna delay text box

SPLASH splash;
uint32 startTime;

wchar_t szTitle[MAX_LOADSTRING + 20];               // The title bar text
...
unsigned int ll_RegAddr = 0;                   // low level register address
unsigned int ll_RegValue = 0;                  // low level register value

int haveSeenPause = 1;

unsigned int spi_Rate = DW_DEFAULT_SPI;            // default SPI rate
unsigned int new_spi_Rate;                             // temp value for update dialog

wchar_t* mode[3] = {(wchar_t*) "Listener", (wchar_t*) "Tag", (wchar_t*) "Anchor"};
wchar_t* tagmode[2] = {(wchar_t*) "Tag Blinking", (wchar_t*) "Tag Ranging"};
wchar_t* anchID[4] = {(wchar_t*) "A1", (wchar_t*) "A2", (wchar_t*) "A3", (wchar_t*) "A4"};

#define RANGING_WITH_STR_LEN    (40)
wchar_t rangingWithString[RANGING_WITH_STR_LEN + 2];
...
long WndProc(void*, unsigned int, unsigned int, long);

int About(void*, unsigned int, unsigned int, long);

int

ChanConfig(void*, unsigned int, unsigned int, long);

int

ContFrameConfig(void*, unsigned int, unsigned int, long);

int

CWModeConfig(void*, unsigned int, unsigned int, long);

int

TagSelectAddress(void* hDlg, unsigned int message, unsigned int wParam, long lParam);

int

TimingConfig(void*, unsigned int, unsigned int, long);

int

RegAccess(void*, unsigned int, unsigned int, long);

int

Reg2File(void*, unsigned int, unsigned int, long);

int

File2Reg(void*, unsigned int, unsigned int, long);

void processLogEvent();

void Toggle_runNotStop();

void setlargedisplayoptions(void* handle);

void plotlargerangetext(void* hDrawWindow, currentRangeingInfo_t* rangeData);

int validateeditboxint(void* handle, int min, int max);

double validateeditboxdouble(void* handle, double min, double max);

int validatedialogboxhex(void* hDlg, int editboxID, unsigned int min, unsigned int max, unsigned int* result);

int validatedialogboxhex64(void* hDlg, int editboxID, uint64_t min, uint64_t max, uint64_t* result);

void setdialogboxhex(void* hDlg, int editboxID, unsigned int value, unsigned int width);

void setdialogboxhex64(void* hDlg, int editboxID, uint64_t value);

unsigned int validatedialogboxdecuint(void* hDlg, int editboxID, unsigned int min, unsigned int max, unsigned int* result);

void setdialogboxdecuint(void* hDlg, int editboxID, unsigned int value, unsigned int width);

void updateblinksseentext();

void setroledropdownlist(void* hDlg, uint8_t roleIndex);

int getroledropdownlist(void* hDlg);
...
uint32 getmstime() {
    ...
    msTime = sysTime.wMilliseconds + sysTime.wSecond * 1000 + sysTime.wMinute * 60 * 1000 +
...
LoadString(hInstance,
IDC_DECARANGING, szWindowClass, MAX_LOADSTRING);
...
Hide();  // hide splash immediately in debug build
#endif
...
.void*, hAccelTable, &msg))
{
if (msg.void* == hDelayTextBox)          // if it is the delay entry text box window
{
...
SetFocus(void*);            // set focus to main window (away from the Text box !)
...
/*                switch (msg.message)
                ...
                case 280 : // undocumented seems to be in time with cursor flashing in edit box
                    break ;
                default:
                    printf("MSG(%i) HW(%i) W(%i) L(%i)\n",msg.message,msg.void*,msg.wParam,msg.lParam) ;
                }
*/
...
instance_run (getmstime())

)    // run it returns flag indicating status report has changed
{
RECT rect;
...
rect);
rect.
bottom = 112;                     // bottom of rectangle covers first four status lines only
if (!displayDebugPanel)                 // if debug panel is not active we have extra status info to display
{
rect.
bottom = movingPanelY - 1;  // bottom of rectangle lower down (just above main panel)
}

InvalidateRect(void*,&

rect,0);          // cause redraw of status info.
...
hInstance = hInstance;
wcex.
hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_DECARANGING));
wcex.
hCursor = LoadCursor(NULL, IDC_ARROW);
wcex.
hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
wcex.
lpszMenuName = MAKEINTRESOURCE(IDC_DECARANGING);
wcex.
...
int inittestapplication(int reset) {
    ...
    if (handle == DWT_ERROR) {
        // error opening spi
        MessageBoxA(NULL, "ERROR - Cannot open a cheetah device or connect to EVK1000 over COM port",
                    "DecaWave Application", MB_OK | MB_ICONEXCLAMATION);
        exit(-1);
    ...
    if ((devID & 0xFFFF0000) != 0xDECA0000) {
        ...
        MessageBoxA(NULL,
                    "ERROR - Failure to read ScenSor ID register, please ensure Cheetah is connected to ScenSor SPI or"
                    " if using USB connection please ensure all Cheetahs are disconnected.", "DecaWave Application",
    ...
    {
        // error seems cheetah read does not see spi of the DecaWave protoscen
        MessageBoxA(NULL, "ERROR - Unsupported Device (not MP), please consult DecaWave for advice. ",
                    "DecaWave Application", MB_OK | MB_ICONEXCLAMATION);
    ...
    if (initComplete == 0) // main init
    {
        ...
        EnableMenuItem(GetMenu(void * ), IDM_REGACCESS,
                       MFS_DISABLED);  // Starting in run mode so disable the reg access menu
    } else {
    ...
    if (instance_mode == ANCHOR) {
        EnableMenuItem(GetMenu(void * ), IDM_DISPCLKOFFSET, MFS_ENABLED);  // Anchor does Clock Offset calculation
        instancesetdisplayclockoffset(viewClockOffset);              // apply change
#if (TAG_ANCHOR_AUTO_LOG == 1)
        setLogging(IDM_LOGALLACC);
#endif
    } else {
        EnableMenuItem(GetMenu(void * ), IDM_DISPCLKOFFSET,
                       MFS_DISABLED);  // Tag/Listener does not do Clock Offset calculation
        viewClockOffset &= ~MF_CHECKED; // uncheck if selected
        CheckMenuItem(GetMenu(void * ), IDM_DISPCLKOFFSET, viewClockOffset);
    ...
    if (initComplete == 0) {
        antennaDelay16 = instancegetantennadelay(
                DWT_PRF_16M);          // MP Antenna Delay at 16MHz PRF (read from OTP calibration data)
        antennaDelay64 = instancegetantennadelay(
                DWT_PRF_64M);          // MP Antenna Delay at 64MHz PRF (read from OTP calibration data)
        if (antennaDelay16 == 0)
            antennaDelay16 = DWT_PRF_16M_RFDLY;    // Set a value locally if not programmed in OTP cal data
        if (antennaDelay64 == 0)
            antennaDelay64 = DWT_PRF_64M_RFDLY;    // Set a value locally if not programmed in OTP cal data
    }
...
{
void* hRoleGroup, hRestartButton;
void* hDelayGroup;
void* hClrCountsButton, hConfigureButton;
...
*/
SendMessage(void*, WM_SETFONT,(unsigned int)

hFont, 1);

panx = 4;
pany = MAIN_PANEL_MIN_Y;
rolx = 16;
roly = 20;
delx = 158;
dely = 20;
...
hMainPanel = CreateWindowEx(0, _T("Static"), NULL, WS_VISIBLE | WS_CHILD | WS_DLGFRAME, panx, pany,
                            MAIN_PANEL_MIN_WIDTH, 150, void * , (HMENU)MAIN_PANEL_ID, NULL, NULL);
SendMessage(hMainPanel, WM_SETFONT, (unsigned int)
hFont, 1);
...
SendMessage(hRoleGroup, WM_SETFONT, (unsigned int)
hFont, 1);

//create the Role Group
hRoleComboBox = CreateWindowEx(0, _T("ComboBox"), _T("Role"),
                               CBS_DROPDOWNLIST | WS_VISIBLE | WS_CHILD,
                               panx + rolx, pany + roly + 20, 80, 164, void * , (HMENU)ROLE_ID, NULL, NULL);
SendMessage(hRoleComboBox, WM_SETFONT, (unsigned int)
hFont, 1);

movingButtons[MB_ROLE].
handle = hRoleComboBox;
movingButtons[MB_ROLE].
xoffs = rolx + 20;
movingButtons[MB_ROLE].
yoffs = roly + 20;

SendMessage(hRoleComboBox, CB_RESETCONTENT,
0, 0);
SendMessage(hRoleComboBox, CB_ADDSTRING,
0,(long) _T("Listener"));
SendMessage(hRoleComboBox, CB_ADDSTRING,
0,(long) _T("Tag"));
SendMessage(hRoleComboBox, CB_ADDSTRING,
...
setroledropdownlist(void*, mode);  //modes 0,1,2 are listener, tag and anchor
}
...
SendMessage(hBlinkAdvise, WM_SETFONT, (unsigned int)
hFont, 1);
}
...
SendMessage(hDelayGroup, WM_SETFONT, (unsigned int)
hFont, 1);

//create the Delay Text Box  (Sub function of void* window, rathar than hDelayGroup so can get at message handle proc easily)

{
TCHAR tempDelayString[20];
// _stprintf(tempDelayString,_T("%0.3f"),antennaDelay) ;
_stprintf_s(tempDelayString,
sizeof(tempDelayString)/sizeof(TCHAR),_T("%0.3f"),antennaDelay);

hDelayTextBox = CreateWindowEx(0, _T("Edit"), tempDelayString,
                               BS_TEXT | WS_VISIBLE | WS_CHILD, // ES_AUTOHSCROLL
                               panx + delx + 21, pany + dely + 20, 85, 20, void * , (HMENU)ANTENNA_DELAY_ID, NULL,
                               NULL);
SendMessage(hDelayTextBox, WM_SETFONT, (unsigned int)
hFont, 1);
//printf("Antenna Delay Text Edit box window handle is %i\n",hDelayTextBox) ;
movingButtons[MB_ANTDELAY].
handle = hDelayTextBox;
movingButtons[MB_ANTDELAY].
xoffs = delx + 21;
movingButtons[MB_ANTDELAY].
yoffs = dely + 20;
...
//create the Restart Button (Sub function of void* window, rathar than hRoleGroup so can get at message handle proc easily)
hRestartButton = CreateWindowEx(0, _T("Button"), _T("Restart"),
                                BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD | WS_TABSTOP,
                                panx + 310, pany + 67, 75, 23, void * , (HMENU)RESTART_BUTTON_ID, NULL, NULL);
SendMessage(hRestartButton, WM_SETFONT, (unsigned int)
hFont, 1);
movingButtons[MB_RESTART].
handle = hRestartButton;
movingButtons[MB_RESTART].
xoffs = 310;
movingButtons[MB_RESTART].
yoffs = 67;

//create the Clear Counters Button (Sub function of void* window, rathar than hPanel so can get at message handle proc easily)
hClrCountsButton = CreateWindowEx(0, _T("Button"), _T("Clear Counters"),
                                  BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD | WS_TABSTOP,
                                  panx + 167, pany + 107, 108, 23, void * , (HMENU)CLEAR_COUNTS_BUTTON_ID, NULL, NULL);
SendMessage(hClrCountsButton, WM_SETFONT, (unsigned int)
hFont, 1);
movingButtons[MB_CEARCOUNTS].
handle = hClrCountsButton;
movingButtons[MB_CEARCOUNTS].
xoffs = 167;
movingButtons[MB_CEARCOUNTS].
yoffs = 107;

//create the Configure Button (Sub function of void* window, rathar than hPanel so can get at message handle proc easily)
hConfigureButton = CreateWindowEx(0, _T("Button"), _T("Configure"),
                                  BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD | WS_TABSTOP,
                                  panx + 310, pany + 107, 75, 23, void * , (HMENU)CONFIGURE_BUTTON_ID, NULL, NULL);
SendMessage(hConfigureButton, WM_SETFONT, (unsigned int)
hFont, 1);
movingButtons[MB_CONFIG].
handle = hConfigureButton;
movingButtons[MB_CONFIG].
xoffs = 310;
movingButtons[MB_CONFIG].
yoffs = 107;

//create the Pause Button (Sub function of void* window, rathar than hPanel so can get at message handle proc easily)
hPauseButton = CreateWindowEx(0, _T("Button"), _T("Pause"),
                              BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD | WS_TABSTOP,
                              panx + 310, pany + 24, 75, 23, void * , (HMENU)PAUSE_BUTTON_ID, NULL, NULL);
SendMessage(hPauseButton, WM_SETFONT, (unsigned int)
hFont, 1);
movingButtons[MB_PAUSE].
handle = hPauseButton;
movingButtons[MB_PAUSE].
xoffs = 310;
movingButtons[MB_PAUSE].
...
movingButtons[MB_STOPLOGGING].
handle = 0;      // dynamically created button not there initially

movingButtons[MB_ACCLOGHIST].
handle = 0;      // dynamically created button not there initially
movingButtons[MB_ACCLOGHIST2].
handle = 0;      // dynamically created button not there initially
...
CheckMenuItem(GetMenu(void*),

IDM_ACCFPALIGN,accumulatorFirstPathAlign);
}


#if (DECA_KEEP_ACCUMULATOR == 0)
                                                                                                                        {
		HMENU hMenu = GetMenu(void*) ;
		...
		EnableMenuItem(hMenu,IDM_ACCMAGONLY,MFS_DISABLED) ;
                                                                                                                        ...
                                                                                                                        {
		EnableMenuItem(GetMenu(void*),IDM_LOGALLACC,MFS_DISABLED) ;
	}
#endif

setlargedisplayoptions(void*);

ShowWindow(void*, nCmdShow);
...
void flagaccumulatorpanelepaintneeded(void) {
    RECT rect;
    GetClientRect(void * , &rect);
    rect.top = DEBUG_PANEL_Y;
    rect.bottom = DEBUG_PANEL_Y + debugPanelHeight;
    InvalidateRect(void * , &rect, 0);      // cause redraw of status info.
...
void setaccplotscale(void* hDrawWindow) {
    RECT rect;

    GetClientRect(hDrawWindow, &rect);

    xscale = ((double) (numPixelsX = rect.right - rect.left)) /
             (plotMaxX - plotMinX);    // (Number of pixels) / ( Value Range)
    yscale = ((double) (numPixelsY = rect.bottom - rect.top)) /
             (plotMaxY - plotMinY);    // (Number of pixels) / ( Value Range)
}
...
/*
void plotranges(void* hDrawWindow, rangesPlotInfo_t *rp)
{
...
void plotaccumulator(void* hDrawWindow, accumPlotInfo_t* ap) {
    ...
    HBRUSH hBrushKeep;
    HBRUSH hBrushTint = CreateSolidBrush(
            RGB(255, 255, 204));                         // brush to tint background when error detected
#endif

    RECT rect;
    int x;

    initaccplotsize(ap->accumSize,
                    (ap->cp[0] == 'O'));                                       // test here determines if data is good
    setaccplotscale(hDrawWindow);
    normaliseScale = 1.0;                                                          // will only scale normalise values
    ...
    hdc = CreateCompatibleDC(NULL);
    hBitmap = CreateCompatibleBitmap(hdcReal, rect.right,
                                     rect.bottom);                 // assuming rectangls starts at 0,0
...
#endif
    i = 2;                                                                         // temp offset
    Rectangle(hdc, rect.left + i, rect.top + i, rect.right - i,
              rect.bottom - i);               // draw rectangle to clear the area
    ...
    if (accumulatorFirstPathAlign) {
        pixOffset = (INT)(ap->hwrFirstPath * xscale - plotLimX * xscale * 3 /
                                                      5.0);        // difference between first path and 1/3 way through in pixels
    ...
    MoveToEx(hdc, scalexpoint(plotMinX), scaleypoint((INT16) 0), NULL);                 // this sets the drawing cursor
    LineTo(hdc, scalexpoint(plotMaxX),
           scaleypoint((INT16) 0));                        // this draws the axis (with default black pen)
...

    {
        int size = (int) (plotMaxX - plotMinX);                                     // figure out size of X axis, and
        if (size > 200)
            markStep = 100;                                            // set markStep for marking off X-axis
        else if (size > 20) markStep = 10;
        else markStep = 1;
    }

    // plot the real values
    hPenKeep = (HPEN) SelectObject(hdc,
                                   hPenReal);                                    // switch on red pen (keep default for restore)
#if (DECA_KEEP_ACCUMULATOR == 1)
    ...
    if ((!accumulatorMagnitudesOnly) || !(ap->hwrFirstPath > 0)) {
        ...
        for (i = minIndx; i <= maxIndx; i++) {
            ...
            if (ind < lastInd) {
                MoveToEx(hdc, scalexpoint(ind) - pixOffset, scaleypoint(ap->accvals[ind].real),
                         NULL);     // this moves the drawing cursor without a line
            } else {
                LineTo(hdc, scalexpoint(ind) - pixOffset,
                       scaleypoint(ap->accvals[ind].real));             // this draws to successive points
            }
            lastInd = ind;
        }
        ...
        for (i = minIndx; i <= maxIndx; i++) {
            ...
            if (ind < lastInd) {
                MoveToEx(hdc, scalexpoint(ind) - pixOffset, scaleypoint(ap->accvals[ind].imag),
                         NULL);     // this moves the drawing cursor without a line
            } else {
                LineTo(hdc, scalexpoint(ind) - pixOffset,
                       scaleypoint(ap->accvals[ind].imag));             // this draws to successive points
            }
            lastInd = ind;
        }
    }
    ...
    for (i = minIndx; i <= maxIndx; i++) {
        ...
        //j = ap->magvals[MAX_ACCUMULATOR_ELEMENTS];
        if (ind < lastInd) {
            MoveToEx(hdc, x = scalexpoint(ind) - pixOffset, y = scaleypoint(ap->magvals[ind]),
                     NULL);     // this moves the drawing cursor without a line
        } else {
            LineTo(hdc, x = scalexpoint(ind) - pixOffset,
                   y = scaleypoint(ap->magvals[ind]));             // this draws to successive points
        }
        if ((accumulatorMagnitudesOnly) && (markStep <= 10))                        // i.r. a close up on magnitudes
        {
            // do something to indicate points
    ...
    {
        MoveToEx(hdc, (x = scalexpoint(ap->windInd) - pixOffset), scaleypoint(plotMinY / 2),
                 NULL);   // this sets the drawing cursor
        LineTo(hdc, x,
               scaleypoint(plotMaxY / 2));                                    // this draws the leading path line
        ...
        //only HW FP
        {
            ...
            if (x > rect.right /
                    2)                                                     // leading path is to right of the area
            {
    ...
    if (ap->hwrFirstPath != -1) {
        ...
        MoveToEx(hdc, (x = scalexpoint(ap->hwrFirstPath) - pixOffset), scaleypoint(plotMinY),
                 NULL);   // this sets the drawing cursor
        LineTo(hdc, x,
               scaleypoint(plotMaxY));                                          // this draws the leading path line
    ...
    if (0 > (minIndx + indOffset)) {
        x = scalexpoint((INT16)(-indOffset));
        count = sprintf_s(buff, sizeof(buff), "%-4.0f", 0.0);
        TextOutA(hdc, 4 + x, numPixelsY / 2 + i, (LPCSTR) buff,
                 count);                      // print min X value displayed
    } else {
        count = sprintf_s(buff, sizeof(buff), "%-4.0f", plotMinX + indOffset);
        TextOutA(hdc, 4, numPixelsY / 2 + i, (LPCSTR) buff,
                 count);                        // print min X value displayed
    }
    ...
    if ((indWrap) && (indWrap < (maxIndx + indOffset))) {
        ...
        // printf("maxIndx(%i),indOffset(%i),indWrap(%i),  x(%i)\n",maxIndx,indOffset,indWrap,x) ;
        count = sprintf_s(buff, sizeof(buff), "%i", indWrap);
        TextOutA(hdc, x - 20, numPixelsY / 2 + i, (LPCSTR) buff,
                 count);                      // print max X value displayed
    } else {
        count = sprintf_s(buff, sizeof(buff), "%4.0f", plotMaxX + indOffset);
        TextOutA(hdc, numPixelsX - 36, numPixelsY / 2 + i, (LPCSTR) buff,
                 count);            // print max X value displayed
    }
...
#if (DECA_BADF_ACCUMULATOR == 1)
    TextOutA(hdc, numPixelsX - 76, 4, ap->cp,
             7);                                           // Frame Error Status, only space for 7 chars
#endif

    if (ap->totalSNR != -99.0f) {
        // for both total SNR and RSL the value is not accurate when RSL > -85
        count = sprintf_s(buff, sizeof(buff), "TotSNR %c %2.1f dBm (%2.1f)", (ap->RSL > -85) ? '>' : '=', ap->totalSNR,
                          ap->totalSNRavg);
    ...
    if (ap->RSL != -99.0f) {
        // for both total SNR and RSL the value is not accurate when RSL > -85
        count = sprintf_s(buff, sizeof(buff), "RSL %c %2.1f dBm (%2.1f)", (ap->RSL > -85) ? '>' : '=', ap->RSL,
                          ap->RSLavg);
...

    {
        ...
        while ((start < plotMaxX + indOffset) || (start < plotMaxX)) {
            MoveToEx(hdc, (x = scalexpoint(start) - pixOffset), scaleypoint(plotMinY * 0.1),
                     NULL);      // this sets the drawing cursor
            LineTo(hdc, x,
                   scaleypoint(plotMaxY * 0.1));                                    // this draws the marker line
    ...
    SelectClipRgn(hdc,
                  NULL);                                                       // return to default clipping area (I hope)
...
void plotlargerangetext(void* hDrawWindow, currentRangeingInfo_t* rangeData) {
    ...
    hLargeFont = CreateFont(textHeight, textWidth, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                            OUT_OUTLINE_PRECIS,
                            CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, VARIABLE_PITCH, TEXT("Impact"));

    hSmallerFont = CreateFont(textHeight / 3, 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                              OUT_OUTLINE_PRECIS,
                              CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, VARIABLE_PITCH, TEXT("Impact"));
...
//    DrawTextA(hdc, (LPCSTR)buff, -1,&rect, CT_CENTER);
    ...
    {
        cnt = sprintf_s(buff, sizeof(buff), "Instant  =   %7.2f m    ",
                        rangeData->idist);    // bottom value is instantenaous range
...
void prevAccButtonDisplay() {
    if (hAccLogHistButton == 0)  // no button
    {
        ...
        hAccLogHistButton = CreateWindowEx(0, _T("Button"), _T(" << "),
                                           ...
                                           8 + 400,
                                           movingPanelY + 65,
                                           47, 25, void * , (HMENU)PREVACC_BUTTON_ID, NULL, NULL);
        ...
        hAccLogHistButton2 = CreateWindowEx(0, _T("Button"), _T(" >> "),
                                            ...
                                            8 + 452,
                                            movingPanelY + 65,
                                            47, 25, void * , (HMENU)NXTACC_BUTTON_ID, NULL, NULL);
        ...
        hAccLogSaveButton = CreateWindowEx(0, _T("Button"), _T("Save ACC"),
                                           ...
                                           8 + 400,
                                           movingPanelY + 105,
                                           99, 25, void * , (HMENU)SAVEACC_BUTTON_ID, NULL, NULL);
        ...
        SendMessage(hAccLogHistButton, WM_SETFONT, (unsigned int)
        hFont, 1);
        SendMessage(hAccLogHistButton2, WM_SETFONT, (unsigned int)
        hFont, 1);
        SendMessage(hAccLogSaveButton, WM_SETFONT, (unsigned int)
        hFont, 1);
...
void debugPanelDisplay(void) {
    ...
    if (displayDebugPanel)                          // create panel if it is needed now.
    {
        hDebugPanel = CreateWindowEx(0, _T("Static"), NULL, WS_VISIBLE | WS_CHILD | WS_DLGFRAME,
                                     ...
                                     void * , NULL, NULL, NULL);
//            SendMessage(hDebugPanel, WM_SETFONT, (unsigned int)(HFONT)GetStockObject(ANSI_FIXED_FONT), 1) ;
        SendMessage(hDebugPanel, WM_SETFONT, (unsigned int)
        hFont, 1);
...
/*
            ...
            hStopLoggingButton = CreateWindowEx(0,_T("Button"),_T("STOP LOGGING"),
                                     ...
                                     75, 52, void*, (HMENU)LOGOFFACC_BUTTON_ID,NULL,NULL);
            // SendMessage(hStopLoggingButton, WM_SETFONT, (unsigned int) hFont, 1) ;
...
*/
            ...
            hStopLoggingButton = CreateWindowEx(0, _T("Button"), _T("STOP LOGGING"),
                                                ...
                                                120, 30, void * , (HMENU)LOGOFFACC_BUTTON_ID, NULL, NULL);
            // SendMessage(hStopLoggingButton, WM_SETFONT, (unsigned int) hFont, 1) ;
            movingButtons[MB_STOPLOGGING].handle = hStopLoggingButton;
...
void Toggle_runNotStop(void) {
    ...
    if (runNotStop)
        EnableMenuItem(GetMenu(void * ), IDM_REGACCESS, MFS_DISABLED);  // When running disable the reg access menu
    else
        EnableMenuItem(GetMenu(void * ), IDM_REGACCESS, MFS_ENABLED);   // when stopped enable the reg access menu
}

// ==============================================================================
// set large display options
void setlargedisplayoptions(void* void*) {
    HMENU hMenu = GetMenu(void * );
    ...
    switch (largeDisplayMode & (LARGE_DISPLAY_MEAN8 | LARGE_DISPLAY_MEAN4)) {
        case 0                   :
            CheckMenuItem(hMenu, IDM_LARGEINST, MF_CHECKED);
            break;    // Set appriopriate tick
        case LARGE_DISPLAY_MEAN8 :
            CheckMenuItem(hMenu, IDM_LARGEAVE8, MF_CHECKED);
            break;
        case LARGE_DISPLAY_MEAN4 :
            CheckMenuItem(hMenu, IDM_LARGEAVE4, MF_CHECKED);
            break;
...
// =========================================================================
//
//  FUNCTION: WndProc(void*, UINT, unsigned int, LPARAM)
//
...
WndProc(void* void*, UINT message, unsigned int wParam, long lParam) {
    int wmId, wmEvent;
...
/*
    ...
    case WM_SETCURSOR : //  32 0x0020
        break;
    default:
        printf ("void* %i, msg %04X, wparam %08X, lparam %08X\n", void*,  message,  wParam,  lParam) ;
        break;
    }
*/
    //if(WM_TIMER != message)
    ...
    switch (message) {
        case WM_COMMAND:
            ...
            if (splash.SHOWING) return DefWindowProc(void * , message, wParam, lParam);
            ...
            switch (wmId) {
                case IDM_ABOUT:
                    DialogBox(hInst, MAKEINTRESOURCE(IDD_ABOUTBOX), void * , About);
                    break;
                ...
                case IDM_CONTFRAME:
                    DialogBox(hInst, MAKEINTRESOURCE(IDD_CONTFRAME), void * , ContFrameConfig);
                    break;

                case IDM_CWMODE:
                    DialogBox(hInst, MAKEINTRESOURCE(IDD_CWMODE), void * , CWModeConfig);
                    break;

                case IDM_CHANSETUP:
                    DialogBox(hInst, MAKEINTRESOURCE(IDD_CHANSETUPDIALOG), void * , ChanConfig);
                    break;

                case IDM_TIMINGSETUP:
                    DialogBox(hInst, MAKEINTRESOURCE(IDD_TIMINGDIALOG), void * , TimingConfig);
                    break;

                case IDM_REGACCESS:
                    DialogBox(hInst, MAKEINTRESOURCE(IDD_REGACCESSDIALOG), void * , RegAccess);
                    break;
				...
				*/
                ...
                case PREVACC_BUTTON_ID:
                    ...
                    SetFocus(
                            void * );                        // set focus to main window (away from the button) to allow key accelerators to work
                    break;
                ...
                case ROLE_ID :
                    ...
                    if (wmEvent == 9) {
                        int new_instance_role = getroledropdownlist(void * );
                        ...
                        switch (new_instance_role) {
                            ...
                            case ANCHOR:
                                if (instgettaglistlength() != 0) {
                                    DialogBox(hInst, MAKEINTRESOURCE(IDD_TAGSELECTDIALOG), void * , TagSelectAddress);
                                    instance_mode = new_instance_role;
                                } else    // no blinks received
                                {
                                    instance_mode = LISTENER;                     // force back to listener
                                    setroledropdownlist(void * , instance_mode);     // and set role drop down list
                                    MessageBoxA(NULL, "ERROR - Can't set anchor mode, no tag blinks received",
                                                "DecaWave Application", MB_OK | MB_ICONEXCLAMATION);
                                }
                                break;
                ...
                default:
                    return DefWindowProc(void * , message, wParam, lParam);
        ...
        case WM_SIZE: {
            ...
            for (i = 0; i < MB_NUM_MOVING_BUTTONS; i++) {
                if (movingButtons[i].handle != 0) {
                    SetWindowPos(movingButtons[i].handle, NULL, 4 + movingButtons[i].xoffs,
                                 newY + movingButtons[i].yoffs, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
                }
            }
            //  resize debug panel
        ...
        case WM_PAINT:
...

            {
                ...
                if (timerID == 0) timerID = SetTimer(void * , 1, 1, NULL);
                ...
                if (!displayDebugPanel) // if debug panel is not active we have extra status info to display
                {
                    ...
                    TextOutA(hdc, 4 + MAIN_PANEL_MIN_WIDTH /* X */, 4 + 3 * 26/* Y */,
                             (LPCSTR) & (s->scr[NUM_BASIC_STAT_LINES][0]), STAT_LINE_LENGTH);
                }
            }
            ...
            if (accumulatorView && (accumulatorInstanceHasNewData || accumulatorPlotRedraw)) {
                if (accumulatorInstanceHasNewData) {
                    const char* cp = instancegetaccumulatordata(
                            &ap[ap_index]);     // instance fills in size and points to its accumulator data.
    ...
    case WM_LBUTTONDOWN:
        {
            ...
            MessageBoxA(void*, str, "Left Button Clicked", MB_OK);
        }
        break;
*/

        case WM_TIMER: {
            timerID = SetTimer(void * , timerID, 1, NULL);
        }
            break;
...
About(void* hDlg, UINT message, unsigned int wParam, long lParam) {
        ...
        case WM_MOUSEMOVE: {
            ...
            if ((xPos > 147) && (xPos < 252) &&
                ...
                if (hVer == 0) {
                    ...
                    hVer = CreateWindowEx(0, _T("Static"), NULL, WS_VISIBLE | WS_CHILD | WS_DLGFRAME, 12, 100, 154, 22,
                                          hDlg, NULL, NULL, NULL);
                    SendMessage(hVer, WM_SETFONT, (unsigned int)
                    hFont, 1);
            ...
            if ((xPos > 23) && (xPos < 38) &&
                (yPos > 34) && (yPos < 50)) {
                if (hTip == 0) {
                    hTip = CreateWindowEx(0, _T("Static"), NULL, WS_VISIBLE | WS_CHILD | WS_DLGFRAME, 14, 88, 152, 50,
                                          hDlg, NULL, NULL, NULL);
                    SendMessage(hTip, WM_SETFONT, (unsigned int)
                    hFont, 1);
                    SetWindowText(hTip,
                                  _T("\x48\x65\x6cl\x6f \x66\x72\x6f\x6d \x42\x69l\x6c\x79 \x61\x6e\x64 \x5ao\x72\x61\x6e\x2e.\x2e  \x57\x65\'\x72\x65 \x72")
                    _T("e\x73\x70on\x73ib\x6c")
                    _T("e f\x6fr \x74hi\x73 a\x70pl\x69")
                    _T("ca\x74io\x6e."));
                }
            } else {
                if (hTip != 0) {
...
void setroledropdownlist(void* hDlg, UINT8 roleIndex) {
    void* dropdownlist = GetDlgItem(hDlg, ROLE_ID);
...
void setdropdownchannel(void* dropdownlist, UINT8 ch) {
    int listLength = SendMessage(dropdownlist, CB_GETCOUNT, 0, 0);
    TCHAR str[32];                                          // temp string buffer for dropdown list entry

    for (int i = 0; i < listLength; i++) {
        SendMessage(dropdownlist, CB_GETLBTEXT, i, (long) str);
        if (_ttoi(str) == ch) {
...
INT8 getdropdownchannel(void* hDlg) {
    TCHAR str[32];                                          // temp string buffer for dropdown list entry
    void* dropdownlist = GetDlgItem(hDlg, IDC_CHANSELECT);
    int sel = SendMessage(dropdownlist, CB_GETCURSEL, 0, 0);
    if (sel == CB_ERR) {
        setdropdownchannel(dropdownlist, 1);
        return 1;
    }
    SendMessage(dropdownlist, CB_GETLBTEXT, sel, (long) str);
    return _ttoi(str);
}
...
void setdropdownattenuation(void* dropdownlist, UINT8 attn) {
...
INT8 getdropdownattenuation(void* hDlg) {
    TCHAR str[32];                                          // temp string buffer for dropdown list entry
    int i;

    void* dropdownlist = GetDlgItem(hDlg, IDC_TXATTSELECT);
    int sel = SendMessage(dropdownlist, CB_GETCURSEL, 0, 0);
    if (sel == CB_ERR) {
        setdropdownchannel(dropdownlist, 1);
        return 1;
...
#define NUM_NS_IDS 5
const short nsID[NUM_NS_IDS] = { /*ID_CONF_PREAM32,*/ ID_CONF_PREAM64, ID_CONF_PREAM128, ID_CONF_PREAM256,
                                                      ID_CONF_PREAM512, ID_CONF_PREAM2048};
const short snapID[NUM_NS_IDS] = {ID_CONF_PREAM64, ID_CONF_PREAM1024, ID_CONF_PREAM1024, ID_CONF_PREAM1024,
                                  ID_CONF_PREAM4096};
...
void allownspreambles(void* hDlg, int allow) {
    void*
    void*;
    int i;
    ...
    if (allow) {
        for (i = 0; i < NUM_NS_IDS; i++) {
            void* = GetDlgItem(hDlg, nsID[i]);                        // get handle to button
            EnableWindow(void * , 1);                                  // enable (black it in)
        }
    } else {
        for (i = 0; i < NUM_NS_IDS; i++) {
            void* = GetDlgItem(hDlg, nsID[i]);                        // get handle to button
            if (SendMessage(void * , BM_GETCHECK, 0, 0) == BST_CHECKED)   // if the button is checked
            {
                SendMessage(void * , BM_SETCHECK, BST_UNCHECKED, 0);     // uncheck it
                SendMessage(GetDlgItem(hDlg, snapID[i]), BM_SETCHECK, BST_CHECKED,
                            0);  // Snap to next highest standard (using table)
            ...
            EnableWindow(void * , 0);                                  // disable (grey it out)
        }
    }
};


void setradioprf(void* hDlg, int pv) {
    void* prfbutton;
...
void setradiodatarate(void* hDlg, int rate, int onOrOff) {
    void* button;
    int id;
    int reqState;
...
void setradiopreamblelength(void* hDlg, int plen, int onOrOff) {
    void* button;
    int id;
    int reqState;
    ...
    switch (plen) {
        case DWT_PLEN_4096:
            id = ID_CONF_PREAM4096;
            break;
        case DWT_PLEN_2048:
            id = ID_CONF_PREAM2048;
            break;
        case DWT_PLEN_1536:
            id = ID_CONF_PREAM1536;
            break;
        case DWT_PLEN_1024:
            id = ID_CONF_PREAM1024;
            break;
        case DWT_PLEN_512:
            id = ID_CONF_PREAM512;
            break;
...
#if (PAC_SELECTION == 1)
                                                                                                                        void setradiopreacqcorsize(void* hDlg,int pacsz,int onOrOff)
{
    void* button ;
    int id, i ;
    int reqState ;
...
void settickbox(void* hDlg, int id, int on) {
    void* button = GetDlgItem(hDlg, id);
    SendMessage(button, BM_SETCHECK, (on ? BST_CHECKED : BST_UNCHECKED), 0);
}

UINT8 gettickbox(void* hDlg, int id) {
    void* button = GetDlgItem(hDlg, id);
    return (UINT8)(SendMessage(button, BM_GETCHECK, 0, 0));
}

void composecodeselectdropdown(void* hDlg, chConfig_t* config) {
    void* hCtrl;
    int listLength;
    ...
    if (config->nsPreambleCode) {
        SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("Non-Standard"));
    ...
    if (config->prf == DWT_PRF_64M)           // see IEEE Std 802.15.4a-2007, Table 39e
    {
        ...
        switch (config->channel) {
            ...
            case 10:
                ...
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 9"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("10"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("11"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("12"));
                break;
            case 4:
            case 7:
            case 11:
                // codes allowed are 17, 18, 19 and 20
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("17"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("18"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("19"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("20"));
                // Also for compatabibility with narrow band implementations can select 1,2,3,4,5, and 6.
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 9 - for interworking"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("10 - for interworking"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("11 - for interworking"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("12 - for interworking"));
                break;
            default:
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("UNKNOWN CHANNEL ???"));
                SendMessage(hCtrl, CB_SETCURSEL, 0, 0);
                return;
        } // end switch
        if (config->nsDPSCodes) {
            // codes allowed are 13 to 16 and 21 to 24 available for DPS.
            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("13 - for DPS"));
            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("14 - for DPS"));
            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("15 - for DPS"));
            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("16 - for DPS"));
            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("21 - for DPS"));
            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("22 - for DPS"));
            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("23 - for DPS"));
            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("24 - for DPS"));
        }
    } else   // see IEEE Std 802.15.4a-2007, Table 39d
    {
        ...
        switch (config->channel) {
            ...
            case 8:
                ...
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 1 "));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 2 "));
                break;
            case 2:
            case 5:
            case 9:
                // codes allowed are 3,4
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 3 "));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 4 "));
                break;
            case 3:
            case 6:
            case 10:
                // codes allowed are 5,6
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 5 "));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 6 "));
                break;
            case 4:
            case 7:
            case 11:
                // codes allowed are 7,8
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 7 "));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 8 "));
                // Also for compatabibility with narrow band implementations can select 1,2,3,4,5, and 6.
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 1 - for ch 1,8 interworking"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 2 - for ch 1,8 interworking"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 3 - for ch 2,5,9 interworking"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 4 - for ch 2,5,9 interworking"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 5 - for ch 4,7,11 interworking"));
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 6 - for ch 4,7,11 interworking"));
                break;
            default:
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T("UNKNOWN CHANNEL ???"));
    ...
    for (int i = 0; i < listLength; i++) {
        SendMessage(hCtrl, CB_GETLBTEXT, i, (long) str);
    ...
    SendMessage(hCtrl, CB_GETLBTEXT, 0, (long) str);                 // get the string
...
INT8 getdropdownpreamblecode(void* hDlg) {
    TCHAR str[64];                                          // temp string buffer for dropdown list entry
    void* hCtrl = GetDlgItem(hDlg, IDC_PCODSELECT);
    int sel = SendMessage(hCtrl, CB_GETCURSEL, 0, 0);
    if (sel == CB_ERR) {
        return -1;   // something is wrong !
    }
    SendMessage(hCtrl, CB_GETLBTEXT, sel, (long) str);
    return _ttoi(str);
}

int

TagSelectAddress(void* hDlg, UINT message, unsigned int wParam, long lParam) {
    int wmId, wmEvent;
    ...
    switch (message) {
        case WM_INITDIALOG: {
            WCHAR tempString[18];
            void* hCtrl = GetDlgItem(hDlg, IDC_TAGSELECT);
            int i, len;
            uint8* taglist;
            ...
            for (i = 0; i < len; i++) {
                ...
                SendMessage(hCtrl, CB_ADDSTRING, 0, (long) tempString);
            }
            SendMessage(hCtrl, CB_SETCURSEL, 0, 0);
        }
            return (int) TRUE;
        ...
        case WM_COMMAND:
            ...
            switch (wmId) {
                case IDOK : {
                    int len;
                    void* dropdownlist = GetDlgItem(hDlg, IDC_TAGSELECT);
...
//
void constraindatarate(void* hDlg, chConfig_t* config) {
    void* button6M8 = GetDlgItem(hDlg, ID_CONF_RATE6M81);
...
CWModeConfig(void* hDlg, UINT message, unsigned int wParam, long lParam) {
    int wmId, wmEvent;
...
ContFrameConfig(void* hDlg, UINT message, unsigned int wParam, long lParam) {
    int wmId, wmEvent;
...
ChanConfig(void* hDlg, UINT message, WPARAM wParam, long lParam) {
    int wmId, wmEvent;
    void* hCtrl;
...
//            EnableWindow(hCtrl,0) ; //don't allow channel selection for Demo SW
            ...
            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 1 - Centre 3.5 GHz (500 MHz)"));  // (long) (LPCTSTR)

            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 2 - Centre 4.0 GHz (500 MHz)"));

            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 3 - Centre 4.5 GHz (500 MHz)"));

            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 4 - Centre 4.0 GHz (900 MHz)"));
            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 5 - Centre 6.5 GHz (500 MHz)"));
            // SendMessage(hCtrl,CB_ADDSTRING,0,(long) _T(" 6 - Centre 7.0 GHz (500 MHz)") );
            SendMessage(hCtrl, CB_ADDSTRING, 0, (long) _T(" 7 - Centre 6.5 GHz (900 MHz)"));
            // SendMessage(hCtrl,CB_ADDSTRING,0,(long) _T(" 8 - Centre 7.5 GHz (500 MHz)") );
            // SendMessage(hCtrl,CB_ADDSTRING,0,(long) _T(" 9 - Centre 8.0 GHz (500 MHz)") );
            // SendMessage(hCtrl,CB_ADDSTRING,0,(long) _T("10 - Centre 8.5 GHz (500 MHz)") );
            // SendMessage(hCtrl,CB_ADDSTRING,0,(long) _T("11 - Centre 8.0 GHz (1.3 GHz)") );
            ...
            switch (wmId) {
                ...
                case IDC_CHANSELECT :      // channel selection might have changed
                    if (wmEvent == CBN_SELCHANGE) {
                        ...
                        composecodeselectdropdown(hDlg,
                                                  &tempConfig);       // compose and select the appriopriate preamble code
                    }
                ...
                case ID_CONF_PRF64MHZ :                                                 // if we get activity on the PRF radio buttons....
                {
                    int val;
                    if (tempConfig.prf !=
                        (val = readradioprf(hDlg)))               // see if the selection has actually changed
                    {
                ...
                case ID_CONF_RATE110K :
                    ...
                    setradiopreamblelength(hDlg, tempConfig.preambleLength,
                                           0);          // clear current PLEN radio button
...
RegAccess(void* hDlg, UINT message, WPARAM wParam, long lParam) {
        ...
        case WM_COMMAND:
            ...
            switch (wmId) {
                case IDOK : {
                    void*
                    void* tmp = GetFocus();
                    if (GetDlgItem(hDlg, IDOK) == void * tmp) {
                        // OKAY Finished Key
                        EndDialog(hDlg, LOWORD(wParam));
                        return (int) TRUE;
                    }
                ...
                case ID_REGWRITE :
                    ...
                    failure = instancelowlevelwritereg(ll_RegAddr, offset,
                                                       ll_RegValue);      // Write the register value
                    if (failure) {
...
TimingConfig(void* hDlg, UINT message, WPARAM wParam, long lParam) {
    int wmId, wmEvent;
...
int validatedialogboxhex(void* hDlg, int editboxID, UINT min, UINT max, UINT* result) {
    int n, d, i;
    UINT v;
    int change = 0;
...
//          int t ;
    ...
    swprintf(tempString, MAX_X_LENGTH + 1, _T("%0*X"), width,
             v);        // format the value with leading zeros if necessary
...
int validatedialogboxhex64(void* hDlg, int editboxID, UINT64 min, UINT64 max, UINT64* result) {
    int n, d, i;
    UINT64 v;
    int change = 0;
...
//          int t ;
    ...
    swprintf(tempString, MAX_X_LENGTH_64 + 2, _T("%0*X"), width,
             v);        // format the value with leading zeros if necessary
    SetDlgItemText(hDlg, editboxID, tempString);                     // and write back
...
//
void setdialogboxhex(void* hDlg, int editboxID, UINT value, UINT width) {
    WCHAR tempString[MAX_X_LENGTH + 2];

    swprintf(tempString, MAX_X_LENGTH + 1, _T("%0*X"), width,
             value);                // format the value with leading zeros
    SetDlgItemText(hDlg, editboxID, tempString);                                 // and write it

} // end setdialogboxhex() ;

void setdialogboxhex64(void* hDlg, int editboxID, UINT64 value) {
    WCHAR tempString[MAX_X_LENGTH_64 + 2];
    uint32 a = value >> 32;
...
UINT validatedialogboxdecuint(void* hDlg, int editboxID, UINT min, UINT max, UINT* result) {
    int n, d, i;
    UINT v;
    int change = 0;
...
//
void setdialogboxdecuint(void* hDlg, int editboxID, UINT value, UINT width) {
    WCHAR tempString[MAX_DECINT_LENGTH + 2];
...
double validateeditboxdouble(void* void*, double min, double max) {
    ...
    got = GetWindowText(void * , inbuffer, MAX_LENGTH - 1);     // read edit box text (assuming hindow is an edit box)
    ...
    SetWindowText(void * , outbuffer);                         // Set it back into window.
...
int validateeditboxint(void* void*, int min, int max) {
    WCHAR inbuffer[MAX_LENGTH];
    WCHAR outbuffer[MAX_LENGTH];
    WCHAR c;
